...
const SELECT = `
  SELECT u.id, u.imei, u.model, u.warna, u.kapasitas,
         ...
         u.tanggal_terjual, u.alasan_flashsale, u.status_jaringan
  FROM units u
  ...
  return withTransaction(async (client) => {
    ...
    await client.query(
      `INSERT INTO units (id, imei, model, warna, kapasitas, supplier_id, branch_id, tanggal_masuk,
                           status, report_qc, catatan, harga_beli, harga_jual, tanggal_terjual, alasan_flashsale, status_jaringan)
      ...
      [
        ...
        data.tanggal_terjual || null, data.alasan_flashsale || null, data.status_jaringan || 'All Provider',
      ]
    );
    return id;
  }).then(getById);
  ...
  return withTransaction(async (client) => {
    ...
    await client.query(
      `UPDATE units SET imei=$2, model=$3, warna=$4, kapasitas=$5, supplier_id=$6, branch_id=$7,
                         ...
                         harga_jual=$13, tanggal_terjual=$14, alasan_flashsale=$15, status_jaringan=$16
       WHERE id=$1`,
      [
        id, data.imei, data.model, data.warna, data.kapasitas, supplierId, branchId,
        ...
        data.status_jaringan || 'All Provider',
      ]
    );
  }).then(() => getById(id));
}
      ...
      await client.query(
        `INSERT INTO units (id, imei, model, warna, kapasitas, supplier_id, branch_id, tanggal_masuk,
         ...
         ON CONFLICT (id) DO UPDATE SET imei=EXCLUDED.imei, model=EXCLUDED.model, warna=EXCLUDED.warna,
           ...
           status_jaringan=EXCLUDED.status_jaringan`,
        [
          ...
          r.status_jaringan || 'All Provider',
        ]
      );
    }
  }).then(list);
