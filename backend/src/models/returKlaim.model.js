...
async function create(data) {
  return withTransaction(async (client) => {
    ...
    await client.query(
      `INSERT INTO retur_klaim (id, tipe, kategori_barang, referensi, imei, deskripsi, alasan, tanggal, status, nilai, pihak_terkait, catatan, deadline, imei_baru, nilai_cas, tanggal_kembali)
      ...
      [
        ...
        data.deadline || null, data.imei_baru || null, data.nilai_cas || 0, data.tanggal_kembali || null,
      ]
    );
    return id;
  }).then(getById);
...
async function update(id, data) {
  await pool.query(
    `UPDATE retur_klaim SET tipe=$2, kategori_barang=$3, referensi=$4, imei=$5, deskripsi=$6, alasan=$7,
                             ...
                             deadline=$13, imei_baru=$14, nilai_cas=$15, tanggal_kembali=$16
     WHERE id=$1`,
    [
      id, data.tipe, data.kategori_barang, data.referensi || null, data.imei || null,
      ...
      await client.query(
        `INSERT INTO retur_klaim (id, tipe, kategori_barang, referensi, imei, deskripsi, alasan, tanggal, status, nilai, pihak_terkait, catatan, deadline, imei_baru, nilai_cas, tanggal_kembali)
         ...
         ON CONFLICT (id) DO UPDATE SET tipe=EXCLUDED.tipe, kategori_barang=EXCLUDED.kategori_barang,
           ...
           pihak_terkait=EXCLUDED.pihak_terkait, catatan=EXCLUDED.catatan, deadline=EXCLUDED.deadline,
        ...
        [
          ...
          r.deadline || null, r.imei_baru || null, Number(r.nilai_cas) || 0, r.tanggal_kembali || null,
        ]
      );
    }
  }).then(list);
